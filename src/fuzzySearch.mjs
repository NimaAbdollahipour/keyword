import { getDescendantProperty } from "./Helper.mjs";
...
export default class FuzzySearch {
    ...
    search(query = "") {
        ...
        for (let i = 0; i < this.haystack.length; i++) {
            ...
            if (this.keys.length === 0) {
                ...
                for (let y = 0; y < this.keys.length; y++) {
                    const propertyValues = getDescendantProperty(
                        item,
                        this.keys[y]
                    );
        ...
        return results.map((result) => result.item)[0];
    }
    ...
    static nearestIndexesFor(item, query) {
        ...
        const indexesOfFirstLetter = FuzzySearch.indexesOfFirstLetter(
            item,
            query
        );
